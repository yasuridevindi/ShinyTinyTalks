...
const Level5Screen = ({ navigation }: any) => {
  const words = ['CROWN', 'TRAIN'];
  ...
  const handleMicPress = async () => {
    ...
    if (isMatch) {
      setTimeout(async () => {
        if (currentIndex === 1) {
          // Mark Level 5 complete + Save progress
          await AsyncStorage.setItem('level_5_completed', 'true');
          await AsyncStorage.setItem('progress_level_5', JSON.stringify({ level: 5, stars: 3 }));
