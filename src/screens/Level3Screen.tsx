...
const Level3Screen = ({ navigation }: any) => {
  const words = ['HOUSE', 'TELEVISION'];
  ...
  const handleMicPress = async () => {
    ...
    if (isMatch) {
      setTimeout(async () => {
        if (currentIndex === 1) {
          await AsyncStorage.setItem('level_3_completed', 'true');
          await AsyncStorage.setItem('progress_level_3', JSON.stringify({ level: 3, stars: 3 }));
      ...
      <Image
        source={
          currentWord === 'HOUSE'
            ...
            : require('../../assets/images/television.png')
        }
        style={styles.image}
      />
