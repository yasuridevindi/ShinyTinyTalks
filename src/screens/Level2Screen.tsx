...
const Level2Screen = ({ navigation }: any) => {
  const words = ['BAG', 'CAP'];
  ...
  const handleMicPress = async () => {
    ...
    if (isMatch) {
      setTimeout(async () => {
        if (currentIndex === 1) {
          // ✅ Save level 2 progress and unlock
          await AsyncStorage.setItem('progress_level_2', JSON.stringify({ level: 2, stars: 3 }));
          await AsyncStorage.setItem('level_2_completed', 'true');
